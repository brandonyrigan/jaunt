...
async function getLongandLat(city) {
	await fetch(
		`https://api.geoapify.com/v1/geocode/search?text=${city}&format=json&apiKey=${api_key}`,
		requestOptions
	)
...
async function getAttractions(long, lat) {
	fetch(
		`https://api.geoapify.com/v2/places?categories=tourism&filter=circle:${long},${lat},5000&bias=proximity:${long},${lat}&limit=20&apiKey=${api_key}`
	)
		.then((response) => response.json())
		.then(async (result) => {
...
async function displayAttractions(attractions) {
	...
	for (let i = 0; i < attractions.length; i++) {
        ...
        <td><a href="https://maps.google.com/?q=${attraction.lat},${attraction.long}" class="btn btn-success">View on Google Maps</a></td>
        `;
		attractionTableBody.appendChild(attractionElement);
	}
}
